...
The gripper is a reusable resource with a capacity of one. Picking something up uses it, putting something down frees it, and a single boolean *gripper-empty* tracks whether it's available or not. It works like a lock; while the *gripper-empty* is false, the robot cannot pick-up anything else, so only one object is ever held at a time. 
